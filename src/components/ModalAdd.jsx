...
export default function ModalAdd({ isOpen, closeModal, judulModal, inputData, endpoints }) {
    ...
    function handleStore(e) {
        // digunakan untuk event onSubmit, agar halaman tidak terefresh ketika klik button 
        e.preventDefault();
        ...
        // axios.post(endpoints['storeInbound'], dataDetail, {
        //     headers: {
        //         'Authorization': 'Bearer ' + localStorage.getItem('access_token')
        //     }
        // })
                                        ...
                                        <div class="mb-6">
                                            {
                                                item.type == "select" ? (
                                                    <div>
                                                        ...
                                                        <select id={index} name={index} class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500  dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"  onChange={handleChange}>
                                                    ...
                                                    <div>
                                                        <label for={index} class="block text-sm font-medium text-gray-900 dark:text-white capitalize mb-3">{index}</label>
                                                        <input type={item.type} name={index} id={index} class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500  dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"  onChange={handleChange}></input>
                                                    </div>
                                                ) 
                                            }
                                        </div>
